    ...
    with demo_col2:
        with st.expander("📥 Download Example Files & Format Guide"):
            ...
            **CSV alternative:**
            ...
            # Example plate CSV
            plate_example = "0.765,0.767,0.768,0.761,0.760,0.755,0.750,0.770,0.767,0.765,0.767,0.769\\n"
            plate_example += "0.738,0.731,0.745,0.762,0.760,0.756,0.756,0.770,0.774,0.766,0.765,0.769\\n"
            plate_example += "0.739,0.735,0.740,0.729,0.723,0.727,0.739,0.737,0.731,0.715,0.690,0.646\\n"
            plate_example += "0.648,0.598,0.547,0.487,0.354,0.185,0.193,0.251,0.346,0.526,0.600,0.627\\n"
            plate_example += "0.636,0.673,0.691,0.726,0.735,0.736,0.750,0.758,0.757,0.757,0.751,0.765\\n"
            plate_example += "0.758,0.763,0.768,0.768,0.769,0.766,0.767,0.766,0.762,0.765,0.751,0.769\\n"
            plate_example += "0.760,0.760,0.767,0.771,0.772,0.766,0.765,0.771,0.764,0.777,0.766,0.772\\n"
            plate_example += "0.778,0.779,0.778,0.777,0.775,0.776,0.771,0.777,0.776,0.773,0.764,0.746"
            
            st.download_button(
                "📥 Example Plate (CSV)",
                plate_example.replace("\\n", "\n"),
                "example_plate.csv",
                "text/csv"
            )
            
            # Example GNPS TSV
            gnps_example = "Compound_Name\\tRT_Query\\tMQScore\\tSpecMZ\\n"
            gnps_example += "Quercetin-3-O-glucoside\\t5.6\\t0.945\\t463.089\\n"
            gnps_example += "Spiraeoside\\t5.8\\t0.899\\t463.088\\n"
            gnps_example += "kaempferol\\t6.1\\t0.885\\t285.040\\n"
            gnps_example += "RUTIN\\t5.5\\t0.870\\t609.146\\n"
            gnps_example += "Isorhamnetin\\t5.9\\t0.823\\t315.051"
            
            st.download_button(
                "📥 Example GNPS (TSV)",
                gnps_example.replace("\\n", "\n").replace("\\t", "\t"),
                "example_gnps.tsv",
                "text/tab-separated-values"
            )
            
            # Example chromatogram CSV
            chrom_example = "RT,Intensity\\n"
            for rt, intensity in generate_demo()['tic'][:50]:
                chrom_example += f"{rt},{intensity}\\n"
            
            st.download_button(
                "📥 Example Chromatogram (CSV)",
                chrom_example.replace("\\n", "\n"),
...
""", unsafe_allow_html=True)        opacity: 0.95;
        margin: 0;
    }
...
with tab1:
    ...
    with col1:
        ...
        if st.button("🎲 Demo Plate", use_container_width=True, key="demo_plate"):
            st.session_state.plate = generate_demo_plate()
            st.success("✅ Demo plate loaded")
    ...
    with col3:
        ...
        if st.button("🎲 Demo GNPS", use_container_width=True, key="demo_gnps"):
            st.session_state.gnps = generate_demo_gnps()
            st.success("✅ Demo annotations loaded")
